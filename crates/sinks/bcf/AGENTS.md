...
- Depends on `axioval-ir` and `openbim-bcf` only. Never the engine, never a
  source adapter; `axioval-ifc` is a dev-dependency so a test can pin
  `IFC_GLOBAL_ID_SCHEME` to the adapter's `IFC_GLOBAL_ID`.
...
- Run `cargo test -p axioval-bcf`; `tests/export.rs` is the contract and reads
  every archive back.
