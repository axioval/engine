...
- Run `cargo test -p axioval-ifc` and strict clippy after changes. `tests/ifc_session.rs` is the production E2E contract; `tests/ifc2x3.rs` pins release binding; `tests/relationships.rs` and `tests/relationship_integrity.rs` pin the relationship and integrity contracts; the facade's `tests/ifc_classification.rs` pins classification selection end to end.
