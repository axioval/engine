...
- `src/attributes.rs` reads direct attributes by the slot name the file's release gives them. Never read a type object's attributes for an occurrence, and never return a measure's number without its unit.
- `src/materials.rs` maps `ifc-material` onto `MaterialService`, IFC4 only until openbimrs/ifc#77; never read material slots here.
- `src/decomposition.rs` walks wholes over `IfcRelationshipService` edge indexes; keep each walk as IDS defines it and refuse ambiguity instead of choosing.
