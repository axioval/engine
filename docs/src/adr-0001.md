...
`axioval-ifc` maps OpenBIM semantics only. `axioval-axiolid` maps source-neutral geometry requests to Axiolid only. They are peer adapters and may be installed independently. There is no combined OpenBIM-Axiolid adapter.
...
- Core crates prohibit OpenBIM, IFC, STEP, ICDD, Axiolid and the legacy runtime dependencies.
