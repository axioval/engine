...
`axioval-rules` contains reusable, vendor-neutral implementations. Vendor identity, proprietary format handling, localized the provider text and oracle-only ordering remain adapters in `vendor/the provider`.
...
7. Record the provider parity and cutover in the migration ledger when applicable.
